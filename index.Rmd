...
Label0 <- c("Regression", "Non-Linear")
RegVar <- c("Generalized Linear Model (GLM)", "Linear Discriminant Analysis (LDA)")
NRegVar <- c("Classification and Regression Trees (CART), "Support Vector Machines (SVM)", "K-Nearest Neighbors (KNN)", "Random Forest (RF)")
types <- data.frame(RegVar, NRegVar, Label0)
kable(types)
...
Before diving into model comparison, let's look at accuracy and out-of-sample (or "out-of-bag") error. Out-of-sample error is estimated in regression algorithms by "mean squared error (MSE)" or "root mean squared error (RMSE)". These would typically be used when the features and outcome are continuous.  In non-regression algorithms "cross-validation" is typically used to accuracy and error. Since the model that ulimately solved the program was non-linear, we'll look at cross-validation is some detail. In cross-validation, we utilize a portion of the training dataset for testing purposes. There are many variants on this. Since our final model uses k-fold cross-validation, I'll use this to explain. 
...
knitr::kable(list("GLM Model_________________", " CART Model_________________", "LDA Model"))
knitr::kable(list(c1$table, "_______________", c2$table, "__________________", c3$table))
knitr::kable(list("SVM Model_________________", " KNN Model_________________", "RF Model"))
...
Label1 <- c("GLM Model", "CART Model", "LDA Model")
cm1 <- data.frame(c1, c2, c3)
kable(cm1)
...
Label2 <- c("SVM Model", "KNN Model", "RF Model")
cm2 <- list(c4, c5, c6)
kable(cm2)
...
Let's intepret the matrices. The x-axis reflects predictions, and the y-axis reflects observations. So in the GLM Model matrix, a value of 4506 in [x1, y1] indicates that in our training set of 19622 records, we predicted that 4506 records were A and we observed that 4506 were A. But if you look at [x1, y2], the value is 1074. This indicates that we predicted 1074 were B, when they should have been classified as A. So values on the diagonal of the matrix are where predictions match the observations. Any other value on the same row is an error. So on the first row, our out of bag error rate is 1074/4506, or around 23.8%. Repeating for the remaining rows:
2: 389/3408 = 11.4%
3: 138/3284 = 4.2%
4: 172/3004 = 5.7%
5: 95/3512 = 2.7%

The total of these errors is 47.8. 


Why do we have zeros "observed" in the regression models!?
...
We can see the RF Model confusion matrix reflects 100% agreement between what we predict in the training set and what we observe in the training set.  With well over 99% accuracy, this was the algo that predicted the correct Classe values for all 20 test observations. We could "resample" the models to aggregate the results, and/or use PCA to reduce the number of features, but both are unnecessary and could not improve the result.
