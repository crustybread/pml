...
Having fit the models, we can look at a breakdown of their respective model accuracy using max(*model*$results$Accuracy). 

``` {r accUracy, echo = FALSE, eval = TRUE}
...
Before we blindly accept this result, let's look at accuracy and out-of-sample (or "out-of-bag") error. Out-of-sample error is estimated in regression algorithms by "mean squared error (MSE)" or "root mean squared error (RMSE)". These would typically be used when the features and outcome are continuous.  In non-regression algorithms "cross-validation" is typically used to accuracy and error. Since the RF model that ulimately solved the program was non-linear, we'll look at cross-validation is some detail. In cross-validation, we utilize a portion of the training dataset for testing purposes. There are many variants on this. Since our final model uses k-fold cross-validation, I'll use this to explain. 
...
Let's start with GLM model. It may seem odd that some columns are all zeroes in the GLM model. This is a case where the model doesn't predict anything other than the A and B level. That can't be right. A quick check of fit.glm$result$accuracy indicates only 40% accuracy. That's not going to help us. But we knew regression models were not well suited for classification problems. While LDA doesn't seem to have this issue, let's dispense with GLM and LDA. 

So let's look at the classification model SVM. This model achived a maximum accuracy of 0.9369935. The value of 5553 in [A, A] indicates that in our training set of 19622 records, we predicted that 5553 records were A and we observed that 5553 were A. But if you look at [A, B], the value is 14.This indicates that we predicted 14 were B, when they should have been classified as A. In fact, everything else on this row other than A should have be classified A. That is, in order to have 100% accuracy, cell [A, A] should be the sum of A (5553), B (14), C (12), D (0) and E (1), or 5580. (As it is, for example, in the RF model.) So values on the diagonal of the matrix are where predictions match the observations. Any value in a row that is not on the diagonal is a prediction error. 
